# Technical Writing Portfolio

**Portfolio by [Your Name]**
*Application for Documentation Engineer at Baseten*

---

## Overview

This portfolio contains three comprehensive technical writing projects that demonstrate my ability to create clear, developer-focused documentation for complex infrastructure and AI/ML systems. Each project showcases different documentation types and writing styles relevant to the Documentation Engineer role.

## Featured Projects

### 1. StreamInfer API Documentation
**Type:** Getting Started Guide + API Reference
**Focus:** Real-time ML inference service documentation

A complete documentation set for a fictional ML inference API, demonstrating:
- Clear onboarding for developers at all levels
- Comprehensive API reference with practical examples
- Code samples in multiple languages
- Architecture and mental model building
- Troubleshooting and best practices

[→ View StreamInfer Documentation](./streaminfer-docs/)

---

### 2. ModelCache Integration Guide
**Type:** Tutorial Series + Configuration Reference
**Focus:** ML model caching and optimization

An in-depth tutorial and reference guide showcasing:
- Step-by-step integration walkthrough
- Complex concept explanation (caching strategies, invalidation)
- Real-world use cases and patterns
- Performance optimization guidance
- Production-ready configuration examples

[→ View ModelCache Guide](./modelcache-guide/)

---

### 3. Production ML Systems Guide
**Type:** Architecture Guide + Best Practices
**Focus:** End-to-end ML deployment and operations

A comprehensive guide to production ML infrastructure, featuring:
- High-level architecture and design decisions
- Infrastructure setup and scaling strategies
- Monitoring, observability, and debugging approaches
- Cost optimization and reliability patterns
- Cross-cutting concerns for production systems

[→ View Production ML Guide](./production-ml-guide/)

---

## About These Projects

Each project represents a different facet of technical documentation that I believe is critical for developer tools:

- **StreamInfer** shows my ability to create reference documentation and quickstart guides that reduce time-to-value
- **ModelCache** demonstrates tutorial writing that builds mental models and guides developers through complex integrations
- **Production ML Guide** showcases my understanding of ML infrastructure and ability to synthesize best practices into actionable guidance

All documentation follows a docs-as-code approach, written in Markdown and structured for easy maintenance and collaboration.

## Skills Demonstrated

- Clear technical writing for complex infrastructure concepts
- API and SDK documentation
- Code examples and sample projects (Python, JavaScript, YAML)
- Multiple documentation types (getting started, reference, tutorials, guides)
- Developer-centric thinking and mental model building
- Docs-as-code best practices
- Understanding of ML/AI deployment concepts

---

## Contact

[Your contact information]

*Created as part of my application for the Documentation Engineer position at Baseten*
